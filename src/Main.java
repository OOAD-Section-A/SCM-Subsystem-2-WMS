...
public class Main {
    public static void main(String[] args) {
        ...
        IWMSRepository mockDbRepo = new IWMSRepository() {
            public boolean validatePurchaseOrder(String po) {
                return true;
            }
        ...
        Product cannedBeans = new Product("P001", "SKU-CANNED-99", "Baked Beans", ProductCategory.DRY_GOODS);
        Product milk = new Product("P002", "SKU-DAIRY-1", "Organic Milk", ProductCategory.PERISHABLE_COLD);

        // --- BEGIN SIMULATION COMMENT-OUT FOR PRODUCTION ---
        ...
        wmsFacade.getTaskEngine().scheduleTask(new CycleCountTask(cannedBeans.getSku(), "ZONE-DRY-BIN-99",
                wmsFacade.getInventoryManager(), auditorService, simulatedBinTags));
        wmsFacade.getTaskEngine()
                .scheduleTask(new InterleavedTask("Worker-JohnDoe", "Aisle 4, Rack B", "Aisle 4, Rack A"));

        System.out.println("\n--- 3. Executing the Task Queue ---");
        wmsFacade.getTaskEngine().executeAllPendingTasks();

        System.out.println("\n--- 4. Upgraded Procurement: 3-Way Match ---");
        wms.models.Supplier dairyFarm = new wms.models.Supplier("S025", "Green Valley Farms", 5, 0.95);
        wms.models.PurchaseOrder po = new wms.models.PurchaseOrder("PO-10023", dairyFarm);
        po.addExpectedItem(milk.getSku(), 50, 2.50);
        wms.models.AdvanceShipmentNotice asn = new wms.models.AdvanceShipmentNotice("ASN-7788", po.getPoNumber(),
                dairyFarm, "2026-04-20");
        asn.addExpectedItem(milk.getSku(), 50);

        wms.controllers.InboundReceivingController dockController = new wms.controllers.InboundReceivingController(
                wmsFacade);
        ...
        Supplier vendorA = new Supplier("S001", "Green Valley Farms", 5, 0.95);
        Supplier vendorB = new Supplier("S002", "National Dairy Corp", 2, 0.80);
        Supplier vendorC = new Supplier("S003", "Local Artisan Milks", 8, 0.99);

        // Populate historical metrics (Quality 1-100, Delivery 1-100, Price 1-100,
        // Service 1-100)
        VendorSelectionEngine.SupplierMetrics metricsA = new VendorSelectionEngine.SupplierMetrics(vendorA, 90, 85, 80,
                95); // Good all-rounder
        VendorSelectionEngine.SupplierMetrics metricsB = new VendorSelectionEngine.SupplierMetrics(vendorB, 75, 95, 90,
                70); // Fast but lower quality
        VendorSelectionEngine.SupplierMetrics metricsC = new VendorSelectionEngine.SupplierMetrics(vendorC, 98, 60, 70,
                90); // High quality, slow delivery
        ...
        System.out.println("\n[A] Outbound Packing (WMS RFID Verification + Packaging Handoff)");
        wms.models.Order outboundOrder = new wms.models.Order("ORD-4455", "CUST-900");
        outboundOrder.addLineItem(milk.getSku(), 2);
        outboundOrder.addLineItem("SKU-GLASS-VASE", 1);

        // Mock product catalog for metadata lookup
        java.util.Map<String, wms.models.Product> catalog = new java.util.HashMap<>();
        catalog.put(milk.getSku(), milk);
        catalog.put("SKU-GLASS-VASE", new wms.models.Product("P003", "SKU-GLASS-VASE", "Decorative Glass Vase",
                wms.models.ProductCategory.HIGH_VALUE));

        // Simulated RFID Scans by the packing worker
        java.util.List<String> rfidScans = java.util.Arrays.asList(milk.getSku(), milk.getSku(), "SKU-GLASS-VASE");

        try {
            // 1. Init Packaging System
            com.scm.packing.mvc.model.PackingModel packingModel = new com.scm.packing.mvc.model.PackingModel();
            com.scm.packing.strategy.PackingStrategyFactory packingFactory = new com.scm.packing.strategy.PackingStrategyFactory();
            com.scm.packing.integration.warehouse.IWarehousePackingIntegration externalPackagingSystem = new com.scm.packing.integration.warehouse.WarehousePackingIntegrationService(
                    packingModel, packingFactory);

            // 2. Init our WMS Services
            wms.services.integration.IExternalPackingService packingAdapter = new wms.services.integration.PackagingSystemAdapter(
                    externalPackagingSystem);
            ...
            // 3. WMS verifies RFID scans, then hands off to Packaging
            packingService.verifyAndDispatch(outboundOrder, rfidScans, catalog, packingAdapter);
        } catch (Exception e) {
            System.err.println("Packing Process Failed: " + e.getMessage());
        }
        ...
        wms.models.Product urgentProduct = new wms.models.Product("SKU-URGENT-55", "Emergency Meds",
                wms.models.ProductCategory.PERISHABLE_COLD);
